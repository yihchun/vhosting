...
def sign_csr(pubkey, webroot, csr, email=None, file_based=False):
...
""".format(
    ...
    os.system("openssl dgst -sha256 -sign user.key -out {} {}".format(reg_file_sig_name, reg_file_name))
    for i in ids:
	os.system("openssl dgst -sha256 -sign user.key -out {} {}".format(i['sig_name'], i['file_name']))
    os.system("openssl dgst -sha256 -sign user.key -out {} {}".format(csr_file_sig_name, csr_file_name))
...
""".format(
    ...
    for i in tests:
	os.system("openssl dgst -sha256 -sign user.key -out {} {}".format(
        i['sig_name'], i['file_name']))
...
""")
    ...
    parser.add_argument("-d", "--webroot", required=True, help="path to your webroot")
    ...
    signed_crt = sign_csr(args.public_key, args.webroot, args.csr_path, email=args.email, file_based=args.file_based)
